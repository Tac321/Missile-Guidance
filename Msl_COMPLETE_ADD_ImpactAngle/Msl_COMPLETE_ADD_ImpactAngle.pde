...
float Gr1=7, Gr2=Gr1, Gr3=Gr2;  // Gr*=7 for pure torque control, = 1 good for LOAL fin deflection.
...
float XNCLOALMAX=0.0, XNCLOALrMAX=0.0;
float switchLOAL= 1;
...
void draw() {  //
  ...
  text(String.format("%.2f", switchLOAL ), 75, 190, 10);
...
// Record maximum angular rates of the A/C
  ...
  text(String.format("%.2f", (switchLOAL)   ), -50, 320-80, 10);// */
...
// End "Moment" computation "function" section
    ...
    ddtheta = cntrlyOld*(1.0-switchLOAL) + (switchLOAL)*( - 1.0/Jy*(Mf1[1].y+Mf2[1].y+Mf3[1].y+Mf4[1].y + Mfd1[1].y+Mfd2[1].y+Mfd3[1].y+Mfd4[1].y+ Mff1[1].y+Mff2[1].y+Mff3[1].y+Mff4[1].y) - 1*dtheta*5.5);  //-5.5 aerodynamic "turning drag"
    ddpsi   = cntrlzOld*(1.0-switchLOAL) + (switchLOAL)*( - 1.0/Jz*(Mf1[1].z+Mf2[1].z+Mf3[1].z+Mf4[1].z + Mfd1[1].z+Mfd2[1].z+Mfd3[1].z+Mfd4[1].z+ Mff1[1].z+Mff2[1].z+Mff3[1].z+Mff4[1].z) - 1*dpsi  *5.5); // */
  ...
  if (guidFlag) {  
    if (abs(b0z)<1.0) { 
      ...
      //XNC2= XNP*()/pow(tgo,2); // pure nc command
       ...
       if (abs(cntrlyOld) > XNCLOALMAX){
         XNCLOALMAX= abs(cntrlyOld);
       }
       if (abs(cntrlzOld) > XNCLOALrMAX){
         XNCLOALrMAX= abs(cntrlzOld);
       }
...
// */
      ...
      PVector.mult(new PVector(switchLOAL, 0, 0), 1)); 
      ...
      Autopilot.applyTranslationAndRotation(PVector.mult(new PVector(normVelocity, dwb, dXLAMD1), 1),        
        ...
        PVector.mult(new PVector(Vc2, XNP, dtheta), 1), // To see target acceleration.
        PVector.mult(new PVector(dpsi, cntrlyOld*TtoXNCScale, cntrlzOld*TtoXNCScale), 1),
        PVector.mult(new PVector(switchLOAL, XNC, XNC2), 1)); 
        DELq= Autopilot.getDELq();
        DELr= Autopilot.getDELr();  // */
